import 'dart:ui';

class AppColor{
  static const Color ink01 = Color(0xFF191D21);
  static const Color ink02 = Color(0xFF656F77);
  static const Color ink03 = Color(0xFFACB8C2);
  static const Color ink04 = Color(0xFFCDD9E3);
  static const Color ink05 = Color(0xFFE8EEF3);
  static const Color ink06 = Color(0xFFFFFFFF);

  static const Color utilityActive = Color(0xFF1814E4);
  static const Color utilitySucces = Color(0xFF23E9B4);
  static const Color utilityInfo = Color(0xFF91D7E0);
  static const Color utilityWarning = Color(0xFFFFAC4B);
  static const Color utilityDanger = Color(0xFFFF5A5A);


  static const Color acccentBlue = Color(0xFFCBE3FF);
  static const Color acccentGreen = Color(0xFFD9FFF8);
  static const Color acccentPurple = Color(0xFFF2E8FF);
  static const Color acccentOrange = Color(0xFFFFD1B6);
  static const Color acccentPink = Color(0xFFFFDBF5);
  static const Color acccentYellow = Color(0xFFFFF3B6);

}